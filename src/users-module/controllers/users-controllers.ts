...
export class UsersController {
    constructor(@inject(UsersServiceMethods) public usersServiceMethods: UsersServiceMethods) {}

    async postUsersController(req: Request, res: Response) { 
    ...
    async getUsersController(req: Request, res: Response) {
        ...
        const result = await this.usersServiceMethods.getUsersByTerm(reqFilter);
        res.status(200).send(result);
    }

    async deleteUserByIdController(req: Request, res: Response) {
        const errors = validationResult(req);
        if(!errors.isEmpty()) {
            const filterErrors = errors.array({onlyFirstError: true}).map((error: any) => ({ 
                message: error.msg.message || error.msg,
