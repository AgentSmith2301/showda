...
describe('/videos', () => {
    ...
    it('- POST does not create the video with incorrect data (no title, no author)', async () => {
        await request(app)
            ...
            .expect(CodeResponsesEnum.Incorrect_values_400, {
                errorsMessages: [
                    {massage: 'title is required', field: 'title'},
                    {massage: 'author is required', field: 'author'},
                ],
            });
    ...
    it('- GET product by ID with incorrect id', async() => {
        await request(app).get('/videos/helloWorld')
            .expect(400);
    });
    ...
    it('+ PUT product by ID with correct data', async() => {
        await request(app)
            ...
            .send({title: 'hello title', author: 'hello author',publicationDate: '2023-01-12T08:12:39.261Z',})
